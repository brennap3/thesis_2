...
# rownames(iraq.data.weekly)

iraq.data.weekly$time<-rownames(iraq.data.weekly)

res.iraq.anom.loc<-dplyr::inner_join(res.iraq.anom.loc,iraq.data.weekly,
                                     by="time") 
...
RAD::ggplot_AnomalyDetection.rpca(res.iraq.anom) + 
  ...
  ggtitle("RAD time series outlier detection algorithm \n detected outbreaks of deaths due to terrorism")+
...
iraq.surus.breakouts.surv<-
          iraq.surus.breakouts %>% dplyr::select(weekstrdate,sum_kill)

iraq.surus.breakouts.surv$method<-"SURUS"

colnames(iraq.surus.breakouts.surv)<-c("time","observed","method")

str(iraq.surus.breakouts.surv)

str(res.iraq.anom.loc)

res.iraq.anom.loc.surv<-
  res.iraq.anom.loc %>% dplyr::select(weekstrdate,sum_kill)

res.iraq.anom.loc.surv$method<-"RAD"

colnames(res.iraq.anom.loc.surv)<-c("time","observed","method")

##head(surv.als)

surv.als2<-rbind(surv.als,iraq.surus.breakouts.surv)
surv.als2<-rbind(surv.als2,res.iraq.anom.loc.surv)
...
alarmplotiraq<-ggplot(surv.als2, aes(x=time,y=observed)) + 
  geom_point(shape=1) + facet_grid(method~.) + 
  scale_color_manual(values=c('green', 'red'))+
  guides(color=FALSE) + theme_bw() +
  ggtitle("Alarm plot for EarC3 and \n Farringtons method and \n SURUS and RAD ")

tsweeklyiraq<-ggplot(iraq.data.weekly, aes(x=weekstrdate, y=sum_kill)) +
  geom_line(color = "blue")+
  ggtitle("Time series plot of the  \n  number of deaths due to terrorism, \n averaged across all weeks in Iraq post invasion")+
  xlab("Interval (week) ")+
  ylab("Number of Deaths")
...
library(cowplot)
?plot_grid
plot_grid(alarmplotiraq, tsweeklyiraq, labels = "", ncol = 1,rel_heights = c(2, 1), align = 'v')
